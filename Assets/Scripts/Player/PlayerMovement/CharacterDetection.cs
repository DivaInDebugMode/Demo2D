...
namespace Player.PlayerMovement // Namespace for managing player movement
{
    ...
    public class CharacterDetection : MonoBehaviour
    {
        ...
        private void IsOnPlatform()
        {
            ...
            switch (stats.MoveVector.x)
            {
                case 0 when detectionStats.IsOnPlatform:
                {
                ...
                default:
                {
                    if ((characterComponents.Rb.velocity.x != 0 || characterComponents.Rb.velocity.y !=0 || !detectionStats.IsOnPlatform && ! detectionStats.IsLedgeDetected && !isDefaultParent) && characterComponents.Rb.transform.parent is not null)
                    {
